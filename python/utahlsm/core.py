...
class UtahLSM:
    ...
    def update(self, dt: float, runtime: float, atm_state: AtmosphericState):
        ...
        qa = self.soil.surface_mixing_ratio(Ts, qs, self.atm_state.p)
    ...
    def run(self, step_count: int, runtime: float) -> SurfaceState:
        ...
        if ( (step_count % self.input.time.step_seb)==0 ):
            ...
            self._compute_fluxes(self.soil_state.T[0],self.soil_state.q[0])
    ...
    def _solve_seb(self):
        ...
        try:
            ...
            self._compute_fluxes(self.sfc_state.Ts, self.sfc_state.qs)
    ...
    def _compute_seb(self, sfc_T: float) -> float:
        ...
        """
        self._compute_fluxes(sfc_T,self.sfc_state.qs);
    ...
    def _solve_smb(self):
        ...
        for _ in range(0,max_iter_flux):
            ...
            E = RHO_A*(gnd_q-self.atm_state.q)*self.sfc_state.ust[0]*fh
    ...
    def _compute_fluxes(self, sfc_T: float, sfc_q: float):
        ...
        self.sfc_state.ghf[0] = K_mid*(sfc_T - self.soil_state.T[1])/(self.input.grid.z[0]-self.input.grid.z[1])
        ...
        for i in range(0,iter_max):
            ...
            if flux_wTv != 0:
                ...
                self.sfc_state.obl[0] = 1e6 # Large positive for neutral
            
            # Bound L to prevent extreme instability/stability
            if (self.input.surface.z_m/self.sfc_state.obl[0] > 5.0): 
                self.sfc_state.obl[0] =  self.input.surface.z_m/5.0
            if (self.input.surface.z_m/self.sfc_state.obl[0] < -5.0): 
                self.sfc_state.obl[0] = -self.input.surface.z_m/5.0
            
            # Check for convergence
            if abs(last_L - self.sfc_state.obl[0]) <= criteria:
        ...
        if not converged:
            self.logger.warning(f"Obukhov length did not converge. Final value = {self.sfc_state.obl[0]}")
