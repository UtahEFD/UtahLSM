...
def _infiltration_capacity(model: Any) -> float:
    """Gravity-limited saturated infiltration capacity K_sat [m/s].

    Mirrors ``_solve_smb``: the conductivity at the wet bracket endpoint
    (theta just below porosity), evaluated for the surface layer. Scaled
    by rho_w it is the maximum mass infiltration rate before
    infiltration-excess runoff is shed.
    """
...
def test_heavy_rain_produces_runoff() -> None:
    """Rain far above the saturated infiltration capacity runs off.

    Only the gravity-limited infiltration capacity rho_w*K_sat enters the
    soil; the remainder is infiltration-excess (Hortonian) runoff.
    """
    RHO_W = 1000.0
...
@pytest.mark.precip
@pytest.mark.smb
def test_rain_above_ksat_runs_off() -> None:
    """Rain above the saturated infiltration capacity runs off.

    Even on dry soil, a rate exceeding rho_w*K_sat exceeds what the
    sealed/saturated surface can transmit downward, so the excess is
    shed as infiltration-excess runoff rather than buffered as storage.
    """
    RHO_W = 1000.0
    model = _make_smb_model(theta_profile=0.10)
    infil_cap = RHO_W * _infiltration_capacity(model)
    P_above_ksat = infil_cap * 10.0

    model.atm_state.precipitation = np.array([P_above_ksat])
    model._solve_smb()

    runoff = float(model.sfc_state.fluxes.runoff[0])
    assert runoff > 0.0
    assert (P_above_ksat - runoff) == pytest.approx(infil_cap, rel=1e-6)
