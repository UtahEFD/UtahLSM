...
@pytest.fixture(params=["clapp-hornberger", "cosby", "rawls-brakensiek"])
...
class TestWaterPotential:
    ...
    def test_brookscorey_water_potential_monotonicity(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
        ...
        theta_min = model.properties.residual[layer] + 0.1 * (
            model.properties.porosity[layer] - model.properties.residual[layer]
        )
        theta = np.linspace(theta_min, model.properties.porosity[layer], 10)
    ...
    def test_campbell_water_potential_saturation(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
        ...
        assert psi_sat < 0.1, (
            f"Water potential at saturation should be near 0: {psi_sat}"
        )

    def test_vangenuchten_water_potential_residual(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_water_potential_bounds(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_water_content_inverts_water_potential(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_moisture_capacity_positive(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
...
class TestHydraulicConductivity:
    ...
    def test_conductivity_moisture_saturation(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_conductivity_moisture_decreases_with_dryness(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_conductivity_moisture_positive(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
        ...
        for ModelClass in SOIL_MODEL_CLASSES:
            ...
            theta_test = np.linspace(
                ...
                + 0.01,  # Avoid singularity at residual
                model.properties.porosity[layer],
                20,
            )
...
class TestThermalProperties:
    ...
    def test_thermal_conductivity_positive(
        ...
        for ModelClass in SOIL_MODEL_CLASSES:
            ...
            assert np.all(K_th > 0.0), f"{ModelClass.__name__}: Non-positive K_th found"
    ...
    def test_thermal_diffusivity_positive(
        ...
        for ModelClass in SOIL_MODEL_CLASSES:
            ...
            assert np.all(D_th > 0.0), f"{ModelClass.__name__}: Non-positive D_th found"
            assert np.all(np.isfinite(D_th)), (
                f"{ModelClass.__name__}: Non-finite D_th found"
            )

    def test_moisture_diffusivity_valid(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
...
class TestSoilParameterConstraints:
    ...
    def test_residual_less_than_porosity(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_porosity_in_physical_range(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_saturated_conductivity_positive(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_heat_capacity_positive(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
...
class TestModelConsistency:
    ...
    def test_all_models_compute_same_layer_count(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
    ...
    def test_moisture_in_physical_bounds(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
        ...
        theta_test = np.linspace(
            model.properties.residual[layer], model.properties.porosity[layer], 5
        )
...
class TestNumericalStability:
    ...
    def test_no_nan_near_saturation(
        self, soil_props: SoilProps, soil_types_to_test: list[str]
    ) -> None:
...
def _make_johansen_mixed_props() -> dict[str, dict[str, float | None]]:
    """Properties dict mixing mineral clay and organic peat."""
    return {
        "clay": {
            "b": 10.55,
            "psi_sat": -0.053,
        ...
        "peat_hemic": {
            ...
            "K_sat": 2.0e-6,
            "ci": 2.5e6,
            # Letts-style peat layers may omit quartz_fraction.
        },
    }
...
class TestJohansenThermalConductivity:
    ...
    def test_johansen_organic_layers_reasonable(self) -> None:
        ...
        model = VanGenuchten(props, ["clay", "peat_hemic"], "test", "johansen")
    ...
    def test_johansen_missing_mineral_quartz_raises(self) -> None:
        """Missing quartz on a mineral layer should fail instead of guessing."""
        props = _make_johansen_clay_props()
        del props["clay"]["quartz_fraction"]

        with pytest.raises(NamelistError, match="quartz_fraction.*clay"):
