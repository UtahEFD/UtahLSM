      ...
      real*8 moistPotential, h, partialPressure, satHum, specHum_gnd
      
      moistPotential = satPotential(1)*
     >     (porosity(1)/gndScalars(1,moistureIndex))**soilExponent(1)   
      
      h = exp(grav*moistPotential
     >     / (waterGasConst*gndScalars(1,temperatureIndex)))
                
      partialPressure = 6.1078d0*exp(17.269d0*
     >     (gndScalars(1,temperatureIndex) - 273.16d0) /
     >     (gndScalars(1,temperatureIndex) - 35.86d0))
      
      satHum = 0.622d0*( partialPressure /
     >     ( pressureScale - 0.378d0*partialPressure) )
      
      specHum_gnd = h*satHum
